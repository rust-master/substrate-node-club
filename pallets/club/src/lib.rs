...
pub mod pallet {
	...
	pub type ClubMember<T: Config> =
		StorageValue<_, BoundedVec<ClubMembers<T>, T::MAX>, ValueQuery>;
	...
	#[pallet::storage]
	#[pallet::getter(fn get_waiting_list)]
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_member_to_club_one(
			...
			ensure_root(origin)?;
			ensure!(Self::get_waiting_list().contains(&member) == true, <Error<T>>::NotAMember);

			let time = <frame_system::Pallet<T>>::block_number();
			let mem = ClubMembers { AccountId: member.clone(), RegisteredTime: time };
